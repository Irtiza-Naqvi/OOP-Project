...
int main()
{
    ...
    {
        std::cerr << "Failed to load font. Check if arial.ttf exists.\n";
        std::cin.get(); // pauses before closing
        return 1;
    }
    ...
    while (window.isOpen())
    {
                    ...
                    if (buttons[i].getGlobalBounds().contains((float)mouse.x, (float)mouse.y))
                    {
                        ...
                        case 1: // View
                            if (recipeAdded)
                                ...
                                status.setString("Add a recipe first.");
                            break;
                        case 2: // Search
                            if (recipeAdded)
                            {
        ...
        else
        {
            ...
            if (inputMode || searchMode || editMode || deleteMode)
            {
                ...
                {
                    if (inputStep == 0)
                        prompt.setString("Enter recipe name:");
                    else if (inputStep == 1)
                        prompt.setString("Enter number of ingredients:");
                    else if (inputStep < 2 + newRecipe.ingredientCount)
                        prompt.setString("Ingredient " + to_string(inputStep - 1) + ":");
                    else
                        prompt.setString("Enter instructions:");
                }
                window.draw(prompt);
